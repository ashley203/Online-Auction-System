...
<body>
	...
	<%
	 ...
	 try {
			...
			if (result.getString("MAX")!=null){
				%>Must bid at least $<%=increment%> over last bid: $<%=result.getString("MAX")%><% 
				bidMin=result.getString("MAX");
			}else{
				str = "SELECT init_price as INIT FROM auction WHERE clothing_id = " + clothing_id;
				result = stmt.executeQuery(str);
				result.next();
				%>Must bid at least $<%=increment%> over initial price: $<%=result.getString("INIT")%>
				<%
	...
	<form method="post" action="checkAutomaticBid.jsp">
		<table>
			<tr>
				<td>Bid Amount</td><td><input type="text" name="bidding_price"></td>
			</tr>
			<tr>
				<td>Upper Limit</td><td><input type="text" name="upper_limit"></td>
			</tr>
			<tr>
				<td>Bid Increment</td><td><input type="text" name="bid_increment"></td>
			</tr>
			<tr>
